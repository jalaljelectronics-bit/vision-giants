// components/admin/TeamFormModal.tsx
...
function toFormState(member: TeamMember | null): TeamFormState {
  if (!member) {
    return { name: '', role: '', photo: '', order: 0 };
  }
  return {
    name: member.name,
    role: member.role,
...
export default function TeamFormModal({
  ...
  const [form, setForm] = useState<TeamFormState>(toFormState(member));
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = isEditing
        ? await adminApi.put<TeamMember>(`/admin/team/${member!.id}`, form)
        : await adminApi.post<TeamMember>('/admin/team', form);

      if (!res.data) throw new Error('No data returned');
      onSaved(res.data);
    } catch {
      setError('Could not save team member. Check the fields and try again.');
    } finally {
      setIsSaving(false);
    }
  }
